...
export class AuthenticationInterceptor implements NestInterceptor {
    ...
    private async setDatabaseConnection(account: any) {
        ...
        if (account['secret']['type'] == CredentialType.FIRE) {
            ...
            await this.configurationsService.set('authorization', JSON.stringify(connection));
        } else {
            let dbName = `${account['number']}-authorization`;
            let connection = {
                type: conn['type'],
                host: conn['host'],
                port: conn['port'],
            ...
            try {
                ...
                console.log('setting authorization configuration session')
                await this.configurationsService.set('authorization', JSON.stringify(connection));
            }
        }
    }
