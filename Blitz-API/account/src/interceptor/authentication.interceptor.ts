...
export class AuthenticationInterceptor implements NestInterceptor {
  ...
  async intercept(context: ExecutionContext, next: CallHandler): Promise<any> {
    try {
      ...
      const externalApiKey = context.switchToHttp().getRequest().headers['x-external-api-key'];
      Logger.warn('API Key not found. Checking for External API Key');

      if (externalApiKey != '' && externalApiKey != null) {
        const account = await this.getAccountByExternalApiKey(externalApiKey);
        if (account != null) await this.setDatabaseConnection(account);
        else throw 'Unauthorized';
      }
  ...
  private async getAccountByExternalApiKey(externalApiKey: string): Promise<Account> {
    return await this.accountService.findByExternalApiKey(externalApiKey);
  }
