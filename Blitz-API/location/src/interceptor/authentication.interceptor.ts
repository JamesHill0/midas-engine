...
export class AuthenticationInterceptor implements NestInterceptor {
    ...
    private async setDatabaseConnection(account: any) {
        ...
        if (account['secret']['type'] == CredentialType.FIRE) {
            ...
            await this.configurationsService.set('location', JSON.stringify(connection));
        } else {
            let dbName = `${account['number']}-location`;
            let connection = {
                type: conn['type'],
                host: conn['host'],
                port: conn['port'],
            ...
            try {
                ...
                console.log('setting location configuration session')
                await this.configurationsService.set('location', JSON.stringify(connection));
            }
        }
    }
